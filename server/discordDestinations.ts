...
export type DiscordNotificationCategory =
  keyof DiscordDestinationSubscriptions;
...
export function emptyDiscordDestinationSubscriptions():
  DiscordDestinationSubscriptions {
  return {
    entry: false,
    exit: false,
    lowLiquidity: false,
...
function allDiscordDestinationSubscriptions():
  DiscordDestinationSubscriptions {
  return {
    entry: true,
    exit: true,
...
function cleanSubscriptions(
  value: unknown,
  current = emptyDiscordDestinationSubscriptions(),
) {
  ...
  return {
    entry:
      supplied.entry === undefined ? current.entry : supplied.entry === true,
    exit:
      supplied.exit === undefined ? current.exit : supplied.exit === true,
    lowLiquidity:
      supplied.lowLiquidity === undefined
        ? current.lowLiquidity
        : supplied.lowLiquidity === true,
    scannerError:
      supplied.scannerError === undefined
        ? current.scannerError
        : supplied.scannerError === true,
    watchlistOnly:
      supplied.watchlistOnly === undefined
        ? current.watchlistOnly
        : supplied.watchlistOnly === true,
    dailySummary:
      supplied.dailySummary === undefined
        ? current.dailySummary
        : supplied.dailySummary === true,
    weeklySummary:
      supplied.weeklySummary === undefined
        ? current.weeklySummary
        : supplied.weeklySummary === true,
  };
}
...
export class DiscordDestinationManager {
  ...
  private publicManaged(destination: StoredDiscordDestination) {
    ...
    return {
      ...
      subscriptions: cleanSubscriptions(destination.subscriptions),
  ...
  publicLegacyDestination(
    ...
    return {
      ...
      avatarUrl: null,
      enabled,
      subscriptions: allDiscordDestinationSubscriptions(),
      createdAt: "",
      updatedAt: "",
  ...
  async create(input: {
    ...
    const destination: StoredDiscordDestination = {
      ...
      enabled: false,
      subscriptions: emptyDiscordDestinationSubscriptions(),
      createdAt: now,
      updatedAt: now,
      lastTestAt: null,
  ...
  async update(
    ...
    return this.mutate((file) => {
      ...
      if (input.enabled !== undefined) {
        if (
          input.enabled &&
          typeof input.enabled === "object" &&
          !Array.isArray(input.enabled)
        ) {
          const routingUpdate = input.enabled as {
            value?: unknown;
            subscriptions?: unknown;
          };
          if (routingUpdate.value !== undefined) {
            destination.enabled = routingUpdate.value === true;
          }
          if (routingUpdate.subscriptions !== undefined) {
            destination.subscriptions = cleanSubscriptions(
              routingUpdate.subscriptions,
              cleanSubscriptions(destination.subscriptions),
            );
          }
        } else {
  ...
  async deliveryTargets(
    category: DiscordNotificationCategory | null,
    allowLegacyAlongsideManaged: boolean,
    allowedDestinationIds?: ReadonlySet<string>,
  ) {
    const file = await this.readFile();
    const enabledManaged = file.destinations.filter(
      (destination) => destination.enabled,
    );
    const managed = enabledManaged
      .filter(
        (destination) =>
          category === null ||
          cleanSubscriptions(destination.subscriptions)[category],
      )
      .filter(
        (destination) =>
          !allowedDestinationIds ||
          allowedDestinationIds.has(destination.destinationId),
      )
    ...
    const includeLegacy =
      Boolean(supplied) &&
      !allowedDestinationIds &&
      (enabledManaged.length === 0 || allowLegacyAlongsideManaged);
    return [
      ...managed,
      ...(includeLegacy
        ? [
